...
void MCDRV_Init_Motors(void)
{
    ...
    /* 6-channel PWM peripheral init for motors */
...
void InitClock(void)
{
    ...
    ui32CyclesNumber =
        ((M2_PWM_DEADTIME * (g_sClockSetup.ui32FastPeripheralClock / 1000000U)) / 1000U); /* max 2047 cycles */
...
//    ADC2->CFG = (ADC_CFG_ADICLK(0U) | ADC_CFG_MODE(1U) | ADC_CFG_ADIV(1U) | ADC_CFG_AVGS(0U) | ADC_CFG_ADTRG(0U));
   ...
   /**************************************/
    ...
    g_sM2AdcSensor.sCurrSec16.ui16RsltRegPhaC = 3U;
    ...
    g_sM2AdcSensor.sCurrSec23.ui16RsltRegPhaA = 3U;
    /* Sector 4,5 - measured currents Ia & Ib */
    ...
    g_sM2AdcSensor.ui16RsltRegVDcb = 5U;
    /* Assign channels and init all pointers */
