...
static void M1_DemoSpeedStimulator(void);
static void M2_DemoSpeedStimulator(void);
/* Demo Position Stimulator */
static void DemoPositionStimulator(void);

static void M1ScalarDemo(void);
static void M2ScalarDemo(void);
...
bool_t bDemoModeSpeed1   = FALSE;
bool_t bDemoModeSpeed2   = FALSE;
bool_t bDemoModePosition = FALSE;

/* Counters used for demo mode */
static uint32_t M1ui32SpeedStimulatorCnt    = 0U;
static uint32_t M2ui32SpeedStimulatorCnt    = 0U;
static uint32_t ui32PositionStimulatorCnt = 0U;

/* Counter for button pressing */
static uint32_t ui32ButtonFilter = 0U;

/* PWM2 submodule 0 compare event has occurred for VAL4 - trigger for M2 ADC conversion */
uint16_t ui16M1AdcConv = 0U;
/* PWM1 submodule 0 compare event has occurred for VAL4 - trigger for M2 ADC conversion */
...
__RAMFUNC(SRAM_ITC_cm7) void ADC1_IRQHandler(void)
{
    ...
    ui16M1AdcConv =  (((PWM2->SM[0].STS & PWM_STS_CMPF_MASK)& 0x8U) >> 3U);
    if(ui16M1AdcConv == 1U) {
    ...
    ui16M2AdcConv =  (((PWM1->SM[0].STS & PWM_STS_CMPF_MASK) & 0x8U) >> 3U);
    if(ui16M2AdcConv == 1U) {
...
__RAMFUNC(SRAM_ITC_cm7) void TMR1_IRQHandler(void)
{
    ...
    /* Demo speed stimulator */
    M1_DemoSpeedStimulator();
    M2_DemoSpeedStimulator();
    ...
    if (GPIO5->ISR & GPIO_ICR1_ICR0_MASK)
    {
        ...
        if (ui32ButtonFilter > 300)
        {
            ...
            else
            {
                ...
                M1_SetAppSwitch(1);
                bDemoModeSpeed         = TRUE;
                M1ui32SpeedStimulatorCnt = 0;
                M2ui32SpeedStimulatorCnt = 0;
            }
        }
    }
...
bool_t bScalarDemo = FALSE;
float_t fltVHzGain = 0.0623999983;
float_t fltFreqCmd = 10;
__RAMFUNC(SRAM_ITC_cm7) void M1ScalarDemo(void)
{
	if (bM1ScalarDemo || bScalarDemo) {
...
__RAMFUNC(SRAM_ITC_cm7) void M2ScalarDemo(void)
{
	if (bM2ScalarDemo || bScalarDemo) {
...
float_t M1prevDemoSpeed=2000.0;
float_t M2prevDemoSpeed=2000.0;
__RAMFUNC(SRAM_ITC_cm7) static void M1_DemoSpeedStimulator(void)
{
    ...
    if (bDemoModeSpeed || bDemoModeSpeed1)
    {
        M1ui32SpeedStimulatorCnt++;
        switch (M1ui32SpeedStimulatorCnt)
        {
            case 10:
                M1_SetAppSwitch(0);
                break;
...
//                break;
            default:
            	if (M1prevDemoSpeed != demoSpeed){
            		M1_SetSpeed(demoSpeed);
            		M1prevDemoSpeed = demoSpeed;
            	}
            	;
            	break;
        }
...
__RAMFUNC(SRAM_ITC_cm7) static void M2_DemoSpeedStimulator(void)
{
    /* Increase push button pressing counter  */
    if (ui32ButtonFilter < 1000)
    {
    ...
    if (bDemoModeSpeed || bDemoModeSpeed2)
    {
        M2ui32SpeedStimulatorCnt++;
        switch (M2ui32SpeedStimulatorCnt)
        {
            case 10:
                M2_SetAppSwitch(0);
                break;
...
//                break;
            default:
            	if (M2prevDemoSpeed != demoSpeed){
            		M2_SetSpeed(demoSpeed);
            		M2prevDemoSpeed = demoSpeed;
            	}
            	;
            	break;
        }
