...
#define __iomux_gpr14_adr       0x400AC038
#define __iomux_gpr16_adr       0x400AC040
#define __iomux_gpr17_adr       0x400AC044
#define __flexram_bank_cfg      0xFEAAAAA5      // 64k OCRAM, 352k DTCM, 96k ITCM
#define __flexram_itcm_size     0xA             // 64kB - not required for such setting
#define __flexram_dtcm_size     0xA             // 384k - not required for such setting
#define __reconfig_init_sp      0x20003FFC      // stack placed at beginning of DTCM (not at top)
                                                // size of stack 16k
...
// NOTE: Initial stack pointer (SP) presented in vector table must be
//       set to correct value considering the following FlexRAM re-configuration
// NOTE: If the value presented in vector table is not considering it then the stack
//       stack pointer (SP) must be re-configured here, e.g.:
    ...
    __asm volatile ("dsb");
    __asm volatile ("isb");
    *((volatile unsigned int *)__iomux_gpr17_adr) = __flexram_bank_cfg;
    *((volatile unsigned int *)__iomux_gpr16_adr) |= 0x4U;
    __asm volatile ("dsb");
    __asm volatile ("isb");
#ifdef FLEXRAM_ITCM_ZERO_SIZE
    *((volatile unsigned int *)__iomux_gpr16_adr) &= 0xFFFFFFFEU;
    __asm volatile ("dsb");
    __asm volatile ("isb");
#endif
#ifdef FLEXRAM_DTCM_ZERO_SIZE
    *((volatile unsigned int *)__iomux_gpr16_adr) &= 0xFFFFFFFDU;
    __asm volatile ("dsb");
    __asm volatile ("isb");
#endif
#ifdef FLEXRAM_XTCM_POWER_OF_TWO_SIZE
    volatile unsigned int tempRead;
    tempRead = *((volatile unsigned int *)__iomux_gpr14_adr);
    tempRead &= 0xFF00FFFF;
    tempRead |= ((__flexram_itcm_size<<16)|(__flexram_dtcm_size<<20));
    *((volatile unsigned int *)__iomux_gpr14_adr) = tempRead;
    __asm volatile ("dsb");
    __asm volatile ("isb");
#else
    volatile unsigned int tempRead;
