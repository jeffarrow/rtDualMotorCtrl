...
static void NxpNci_FillInterfaceInfo(NxpNci_RfIntf_t* pRfIntf, uint8_t* pBuf)
{
    ...
    case (MODE_POLL | TECH_PASSIVE_NFCF):
        ...
        /* In case of T3T tag, save card IDm for further operation */
        //RW_NDEF_T3T_SetIDm(pRfIntf->Info.NFC_FPP.SensRes);
#endif
        break;
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if((RfIntf.Interface = INTF_ISODEP) && Answer[0] == 0x10)
            {
                ...
                memcpy(&tmp[tmpSize], &Answer[3], Answer[2]);
                tmpSize += Answer[2];
                /* Compute all chanined frame into one unique answer */
...
do
    {
        if(NxpNci_WaitForReception(Answer, sizeof(Answer), &AnswerSize, TIMEOUT_1S) == NXPNCI_ERROR) return NXPNCI_ERROR;
