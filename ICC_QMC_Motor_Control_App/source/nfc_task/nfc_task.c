...
void nfc_init_task(void *pvParameters);
void BOARD_InitNfcPins(void);
void BOARD_DeinitNfcPins(void);

const uint8_t nfcMotorID1[10] = NFC_MOTOR_ID1;
const uint8_t nfcMotorID2[10] = NFC_MOTOR_ID2;
const uint8_t nfcMotorID3[10] = NFC_MOTOR_ID3;
...
void NdefPush_Cb(unsigned char *pNdefRecord, unsigned short NdefRecordSize) {
    ...
    /* Wait 1s before resending the same message*/
    Sleep(1000);
 ...
 ******************************************************************************/
/*!
 * @brief nfc_task function
 */
void nfc_task(void *pvParameters)
{
  EventBits_t eventNfcIgnoreBit;
  
    NxpNci_RfIntf_t RfInterface;
    
    PRINTF("nfc_task entered.\r\n");
	...
	PRINTF("Error: cannot connect to NXPNCI device\n");
        PRINTF("NFC task bypassed\n");
        if (xEventGroupSetBits(eventGroupMotorControlData,EVENT_NFC_ID_LCD_SUCCESS) == pdPASS);
        BOARD_DeinitNfcPins();
        vTaskDelete(NULL);
        return;
    }
    ...
    while(1)
    {
        ...
        {
          bool correctMotorId = false;
          task_nfc_reader(RfInterface);
          if (memcmp(&RfInterface.Info.NFC_APP.NfcId, nfcMotorID1, RfInterface.Info.NFC_APP.NfcIdLen) == 0)
          {
            correctMotorId = true;
          }
          if (memcmp(&RfInterface.Info.NFC_APP.NfcId, nfcMotorID2, RfInterface.Info.NFC_APP.NfcIdLen) == 0)
          {
            correctMotorId = true;
          }
//          if (memcmp(&RfInterface.Info.NFC_APP.NfcId, nfcMotorID3, RfInterface.Info.NFC_APP.NfcIdLen) == 0)
//          {
//            correctMotorId = true;
//          }
          eventNfcIgnoreBit = xEventGroupWaitBits(eventGroupMotorControlData,
                                                  EVENT_NFC_ID_IGNORE,
                                                  pdTRUE,
                                                  pdFALSE,
                                                  0);
          if (correctMotorId || (eventNfcIgnoreBit & EVENT_NFC_ID_IGNORE))
          {
            PRINTF("\nMotor identificaiton successfull.\n");
            if (xEventGroupSetBits(eventGroupMotorControlData,EVENT_NFC_ID_LCD_SUCCESS) == pdPASS);
            BOARD_DeinitNfcPins();
            vTaskDelete(NULL);
          }
          ...
          {
            if (xEventGroupSetBits(eventGroupMotorControlData,EVENT_NFC_ID_LCD_FAIL) == pdPASS);
          }
        }
        else
#endif // if defined RW_SUPPORT
